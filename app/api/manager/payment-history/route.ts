...
export async function GET(request: NextRequest) {
  try {
    const memberStatus = String(
      request.nextUrl.searchParams.get("memberStatus") ?? "",
    )
      .trim()
      .toLowerCase();
    ...
    return successResponse({
      ...
      data: plainHistories.map((item) =>
        serializeHistory(item, { usersById, plansById }),
      ),
    });
  } catch (error) {
    console.error("GET MANAGER PAYMENT HISTORY ERROR:", error);
    return errorResponse("Gagal mengambil riwayat pembayaran", 500);
  }
}
